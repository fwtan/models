...
def main(argv):
  ...
  index_list = read_file(cmd_args.dataset_file_path)
  num_images = len(index_list)
  print('done! Found %d images' % num_images)

  # Compose list of image paths.
  image_paths = [os.path.join(cmd_args.images_dir, index_image_name.split(',')[0])     for index_image_name in index_list]
  image_names = [os.path.splitext(os.path.basename(index_image_name.split(',')[0]))[0] for index_image_name in index_list]
