...
flags.DEFINE_string('d1m_file_path', '/tmp/revisitop1m.txt', 'Dataset file for the one-million dataset, in .txt format.')
...
def _ReadDelgGlobalDescriptors(input_dir, image_list):
  ...
  Returns:
    global_descriptors: NumPy array of shape (len(image_list), D), where D corresponds to the global descriptor dimensionality.
  """
...
def main(argv):
  ...
  d1m_index_list = [osp.splitext(osp.basename(x))[0] for x in read_file(FLAGS.d1m_file_path)]
  index_list = index_list + d1m_index_list
