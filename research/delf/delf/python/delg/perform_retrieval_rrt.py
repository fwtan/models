...
from delf.python.detect_to_retrieve import image_reranking_rrt as image_reranking

FLAGS = flags.FLAGS

flags.DEFINE_string('dataset_file_path', '/tmp/gnd_roxford5k.mat', 'Dataset file for Revisited Oxford or Paris dataset, in .mat format.')
flags.DEFINE_string('query_features_dir', '/tmp/features/query', 'Directory where query DELG features are located.')
flags.DEFINE_string('index_features_dir', '/tmp/features/index', 'Directory where index DELG features are located.')
flags.DEFINE_string('medium_feature_path', '/tmp/features/index', 'Directory where index DELG features are located.')
flags.DEFINE_string('hard_feature_path', '/tmp/features/index', 'Directory where index DELG features are located.')
flags.DEFINE_boolean('use_geometric_verification', False, 'If True, performs re-ranking using local feature-based geometric verification.')
flags.DEFINE_float('local_descriptor_matching_threshold', 1.0, 'Optional, only used if `use_geometric_verification` is True. Threshold below which a pair of local descriptors is considered a potential match, and will be fed into RANSAC.')
flags.DEFINE_float('ransac_residual_threshold', 20.0, 'Optional, only used if `use_geometric_verification` is True. Residual error threshold for considering matches as inliers, used in RANSAC algorithm.')
flags.DEFINE_integer('num_local_descriptors', 1000, 'The number of local descriptors.')
flags.DEFINE_boolean('use_ratio_test', False, 'Optional, only used if `use_geometric_verification` is True. Whether to use ratio test for local feature matching.')
flags.DEFINE_string('output_dir', '/tmp/retrieval', "Directory where retrieval output will be written to. A file containing metrics for this run is saved therein, with file name 'metrics.txt'.")
...
def main(argv):
  ...
  num_local_descriptors = int(FLAGS.num_local_descriptors)
  ...
  medium_rrt_feats = pickle_load(FLAGS.medium_feature_path)
  hard_rrt_feats = pickle_load(FLAGS.hard_feature_path)
  ...
  for i in range(num_query_images):
    ...
    if FLAGS.use_geometric_verification:
      medium_ranks_after_gv[i], cur_t = image_reranking.RerankByGeometricVerification(
          ...
          num_local_descriptors=num_local_descriptors,
          ransac_seed=0,
          ...
          rrt_feats = medium_rrt_feats,
          query_id = i)
      total_time += cur_t
      hard_ranks_after_gv[i], cur_t2 = image_reranking.RerankByGeometricVerification(
          input_ranks=ranks_before_gv[i],
          initial_scores=similarities,
          query_name=query_list[i],
          index_names=index_list,
          ...
          ransac_residual_threshold=FLAGS.ransac_residual_threshold,
          use_ratio_test=FLAGS.use_ratio_test,
          rrt_feats = hard_rrt_feats,
          query_id = i)
      total_time += cur_t2

    elapsed = (time.time() - start)
    print('done! Retrieval for query %d took %f seconds %f' % (i, elapsed, cur_t))

  print('total_time', total_time/(2*num_query_images))
  ...
  pickle_save(os.path.join(FLAGS.output_dir,'medium_nn_inds.pkl'), medium_ranks_after_gv)
  pickle_save(os.path.join(FLAGS.output_dir,'hard_nn_inds.pkl'), hard_ranks_after_gv)
